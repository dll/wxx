...
func TestKBService_List(t *testing.T) {
	...
	list, total, err := svc.List("", "", "published", "", 1, 10)
	if err != nil {
...
func TestKBService_List_Pagination(t *testing.T) {
	...
	// 每页 1 条
	list, total, err := svc.List("", "", "published", "", 1, 1)
	if err != nil {
		t.Fatalf("List 失败: %v", err)
	}
	if total != 3 {
...
func TestKBService_Get(t *testing.T) {
	...
	kb, err := svc.Get("policy-scholarship-2026")
	if err != nil {
...
func TestKBService_Update(t *testing.T) {
	...
	// 先创建
	created, _ := svc.Create(&model.KBCreateRequest{
		ResourceType: "FAQ",
		OwnerScope:   "college",
		RoleScope:    `["student"]`,
		Title:        "原始标题",
	...
	updated, err := svc.Update(created.ResourceID, req, "editor")
	if err != nil {
...
func TestKBService_Update_NotFound(t *testing.T) {
	...
	_, err := svc.Update("nonexistent", &model.KBUpdateRequest{Title: "x"}, "admin")
	if err == nil {
