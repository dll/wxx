...
var projectRoles = setOf("co_owner", "member", "mentor", "reviewer", "platform_operator")
...
func (h *VOPCHandler) InviteMember(c *gin.Context) {
	...
	var n int
	_ = tx.QueryRow(`SELECT COUNT(*) FROM vopc_project_members WHERE project_id=? AND user_id=? AND status='active'`, id, in.UserID).Scan(&n)
	if n > 0 {
		c.JSON(409, gin.H{"code": 409, "message": "用户已是项目成员"})
		return
	}
	_ = tx.QueryRow(`SELECT COUNT(*) FROM vopc_invitations WHERE project_id=? AND invitee_user_id=? AND status='pending'`, id, in.UserID).Scan(&n)
	if n > 0 {
...
func (h *VOPCHandler) ListArtifactVersions(c *gin.Context) {
	...
	defer tx.Rollback()
	rows, err := tx.Query(`SELECT v.id,v.version,v.source_kind,v.source_ref,v.checksum,v.release_notes,v.created_by,v.created_at FROM vopc_artifact_versions v JOIN vopc_artifacts a ON a.id=v.artifact_id WHERE v.artifact_id=? AND a.project_id=? ORDER BY v.id DESC`, aid, id)
	if err != nil {
	...
	for rows.Next() {
		var vid, by int64
		var version, kind, ref, sum, notes, created string
		if rows.Scan(&vid, &version, &kind, &ref, &sum, &notes, &by, &created) != nil {
			serverError(c, "版本读取失败")
			return
		}
		items = append(items, gin.H{"id": vid, "version": version, "source_kind": kind, "source_ref": ref, "checksum": sum, "release_notes": notes, "created_by": by, "created_at": created})
	}
	c.JSON(200, gin.H{"code": 0, "data": items})
}
func (h *VOPCHandler) CreateArtifactVersion(c *gin.Context) {
	...
	res, err := tx.Exec(`INSERT INTO vopc_artifact_versions(artifact_id,version,source_kind,source_ref,checksum,release_notes,created_by) VALUES(?,?,?,?,?,?,?)`, aid, in.Version, in.SourceKind, in.SourceRef, strings.TrimSpace(in.Checksum), strings.TrimSpace(in.Notes), u.UserID)
	if err != nil {
		if strings.Contains(strings.ToLower(err.Error()), "unique") {
	...
	_, _ = tx.Exec(`UPDATE vopc_artifacts SET updated_at=CURRENT_TIMESTAMP WHERE id=?`, aid)
...
func (h *VOPCHandler) SubmitMilestone(c *gin.Context) {
	...
	if in.ReviewerID != nil {
		var count int
		_ = tx.QueryRow(`SELECT COUNT(*) FROM vopc_project_members WHERE project_id=? AND user_id=? AND status='active' AND project_role IN ('mentor','reviewer','platform_operator')`, id, *in.ReviewerID).Scan(&count)
		if count != 1 {
	...
	for _, vid := range in.VersionIDs {
		var count int
		_ = tx.QueryRow(`SELECT COUNT(*) FROM vopc_artifact_versions v JOIN vopc_artifacts a ON a.id=v.artifact_id WHERE v.id=? AND a.project_id=?`, vid, id).Scan(&count)
		if count != 1 {
...
func (h *VOPCHandler) ReviewMilestone(c *gin.Context) {
	...
	if !allowed {
		var role string
		_ = tx.QueryRow(`SELECT project_role FROM vopc_project_members WHERE project_id=? AND user_id=? AND status='active'`, id, u.UserID).Scan(&role)
	...
	if _, err = tx.Exec(`UPDATE vopc_milestone_submissions SET status=?,reviewed_at=CURRENT_TIMESTAMP WHERE id=? AND status='pending'`, next, sid); err != nil {
		serverError(c, "评审保存失败")
		return
	}
	if next == "passed" {
		...
		_, _ = tx.Exec(`UPDATE vopc_milestones SET status='passed',review_note=? WHERE project_id=? AND stage=?`, in.Note, id, current)
	}
