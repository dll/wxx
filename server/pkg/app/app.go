...
func initAppWithConfig(cfg *config.Config) (http.Handler, error) {
	...
	educationHandler := handler.NewEducationHandler(db, repository.NewMentalHealthRepo(db), repository.NewCareerRepo(db), repository.NewHealthRepo(db), repository.NewHealthActivityRepo(db), repository.NewStudyRepo(db))
