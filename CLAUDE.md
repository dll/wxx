...
- **前端部署**：前端已迁移至 Cloudflare Pages，唯一正式访问入口为 `https://wxx-agent.pages.dev`。后端仍由 Cloudflare Pages Functions 代理到 Vercel Serverless，详见 `docs/蔚小芯前端重新部署.md`。不要再使用已停用的 `wxx.pydaydayup.xyz` 作为发布入口
