...
部署链路专项修复：停用并移除 Vercel 后端 workflow；腾讯云后端发布改为 runner 本地 gzip + OpenSSH/SCP 直传，增加上传 SHA256 校验、连接保活、15 分钟上传超时和 5 分钟远程命令超时，远端继续执行数据库备份、原子替换、迁移验证与健康检查。修复此前 scp-action tar 挂起及 ssh-deploy/rsync 临时文件 code 23 问题。
