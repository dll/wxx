...
用户画像诚实空态专项增量：学生数字孪生雷达图不再把无数据维度画成 0 分拉向圆心（视觉伪短板），改为跳出数据多边形、在外圈画空心圆提示“数据积累中”，且可用维度不足 3 个时不画多边形避免视觉误导；综合状态标签在 0 个维度有记录时显示“数据积累中”而非伪“待提升”；新增 `twin_honest_state_test.dart` 契约测试锁定 data_available 反序列化与全空画像行为。通过 Flutter 全量 test（38 项）、analyze 与基线持平（54 条 info 零新增）、Go 定向包测试全绿。

部署链路专项修复：停用并移除 Vercel 后端 workflow；腾讯云后端发布改为 runner 本地 gzip + 8MiB 分块 SSH 上传，先做远端 shell/磁盘预检，每块 120 秒超时并最多重试 3 次，远端合并后做 SHA256 校验；整体仍有连接保活和 5 分钟远程命令超时，继续执行数据库备份、原子替换、迁移验证与健康检查。分块 SSH 不依赖 scp/SFTP subsystem，规避腾讯云镜像大流传输通道建连后无数据进度；同时修复此前 scp-action tar 挂起及 ssh-deploy/rsync 临时文件 code 23 问题。
