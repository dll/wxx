...
## 实施进度
- [x] **后端已完成并冒烟验证（2026-08-15）**：迁移 087 + capability + repo+service+handler + 路由；学生自报/教辅录入/教辅审核/书记大屏全校+本院聚合均验证通过（就业率/考研率真实点亮）。
- [x] **前端已完成（2026-08-15）**：书记教育成果大屏页（全生命周期：竞赛/入党/学业/协同/去向+真实/待接入badge）+ 毕业去向登记页（学生自报/教辅录入+审核）+ 能力入 profile 管理/教辅服务入口；`flutter analyze` 0 error。
