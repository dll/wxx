...
## 八、图片型 PDF 解析增强（P1 已完成）

解决「知识上传时扫描件/图片型 PDF 解析结果仅为分页标记空壳、质量评估误判为有效」的问题：
...
- **前端引导**：解析结果卡片新增警告横幅（tertiaryContainer 配色），SnackBar 即时提示警告内容并引导用户核对；`/documents/formats` 接口新增 `tips` 字段说明 OCR 限制。
