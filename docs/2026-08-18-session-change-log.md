...
- 移除学生首页、个人档案、悬浮菜单和角色菜单中的数字人/数字孪生动画入口；原深链改为“学习数据”普通列表，仅展示可解释指标。
- 教师访问 `/enrollment` 时进入授课概览；教师课程查询在审核关系为空时回退读取 `course_schedules.teacher`，按账号姓名匹配并按课程/学期去重，成绩与作业写入审核边界不变。
- 新增教师课表回退单测，验证 206004（刘东良）可按课表教师姓名显示历史导入课程。
