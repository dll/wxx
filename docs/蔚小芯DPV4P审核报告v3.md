...
以下 handler 仍使用 `gin.H{}` 而非 `model.ErrorResponse`，非阻断但建议统一：

- `counselor_handler.go`
...
| 兜底处理 | **通过** |
| 管道顺序（结构化→FTS→拼装→LLM） | **部分通过** — 缺结构化优先 |
| LLM 调用前检查上下文 | **部分通过** — 空上下文仍调用 LLM |

### 4.2 残留问题

| 级别 | ID | 位置 | 描述 |
|------|----|------|------|
| 中 | MED-KB1 | `docs/context-engine.md:7` | "结构化优先"步骤代码未实现 |
| 中 | MED-KB2 | `chat_service.go:183-205` | 搜索+Agent 空结果时仍调 LLM |
| 低 | LOW-KB1~7 | `process_agent.go` / `policy_agent.go` / `merger.go` / `chat_service.go` | 置信度硬编码、空来源、安全过滤顺序等 7 项低优先级优化 |
...
| MED-Q1 | `kb_handler.go` 全部错误路径 | 错误响应缺 TraceID | ⚠️ 未修复 |
| MED-Q2 | 多处 handler | `err.Error()` 暴露（同安全 WARN-01~05） | ⚠️ 未修复 |
| MED-Q3 | `frontend/api_service.dart:39` | 401 回调后错误继续传播 | ⚠️ 未修复 |
| MED-Q4 | `frontend/knowledge_provider.dart` | 无 `dispose()` | ⚠️ 未修复 |
...
| # | 组件 | 问题 | 估时 |
|---|------|------|------|
| 7 | `counselor_handler.go` / `notification_handler.go` / `upload_handler.go` | `gin.H{}` 非标准错误格式 | 3h |
| 8 | 知识管道 MED-KB1~2 + LOW-KB1~7 | 知识管道优化 | 6.5h |
| 9 | `kb_handler.go` 错误路径 | 缺 TraceID（MED-Q1） | 1h |
| 10 | `frontend/` | 前端错误处理 + dispose（MED-Q3~4） | 2h |
| 11 | 文档更新 | API 契约 / RBAC 矩阵 / 部署文档对齐 | 3h |
| 12 | 基础设施 | 绑定自定义域名、清除构建产物历史 | 2h |
...
|------|---------|---------|---------|------|
| 架构合规 | 4/10 | 7/10 | **9/10** | ↑ 2 — 全部 handler 违规消除 |
| 安全审计 | 5/10 | 7/10 | **7/10** | — 无新增安全变化 |
| 知识管道 | 6/10 | 7/10 | **7/10** | — 无新增变化 |
| 代码质量 | 5/10 | 7/10 | **7/10** | — 无新增变化 |
| 基础设施 | — | — | **7/10** | 新增维度 — Cloudflare Pages 部署完成，缺自定义域名 |
| 文档完整性 | 7/10 | 7/10 | **7/10** | — 已更新 v3 报告 |
| **总分** | **5.4/10** | **7.0/10** | **7.4/10** | **↑ 0.4 分** |
...
> **整体评级：⚠️ 仍需改善（7.4/10）**  
> **v3 进步**：全部 5 个阻断级（P0）已修复完成，架构合规从 7→9 分。  
> **当前重点**：`err.Error()` 批量暴露（6.5h）和用户 API 密钥明文存储（4h）是最大的残余安全风险。  
> **基础设施**：Cloudflare Pages + Turso 部署方案已验证可用，建议绑定自定义域名确保访问稳定性。  
> **建议**：优先消除 `err.Error()` 泄露和密钥加密存储，再推进知识管道和文档更新。
