...
class _ProfilePageState extends State<ProfilePage> {
      ...
      children: [
        ...
        Card(
          ...
          child: ListTile(
            ...
            subtitle: const Text('v${ReleaseConfig.version} · 滁州学院计算机学院'),
            trailing: const Icon(Icons.chevron_right),
            onTap: () {
              context.push('/about');
            },
  ...
  List<_ProfileFeature> _featuresFor(String? role) => [
        ...
        if (role == 'student' || role == 'student_union') ...[
          ...
          const _ProfileFeature('club', '学生服务', Icons.groups_outlined, '社团生活',
              '社团加入与活动参与', '/club'),
        ],
        if (role == 'counselor') ...[
        ...
        if (CapabilityUtils.has(Capability.counselorKbWrite))
          const _ProfileFeature('p_manage', '办事服务', Icons.edit_note, '办事管理',
              '新增、编辑、发布和导出办事流程', '/process-manage'),
        if (CapabilityUtils.has(Capability.counselorKbReview))
          const _ProfileFeature('p_review', '办事服务', Icons.rate_review_outlined,
              '办事审核', '审核学校、学院管理员提交的办事流程', '/process-review'),
        if (_canAccessEmotion(role))
          const _ProfileFeature('emotion', '管理服务', Icons.warning_amber_rounded,
